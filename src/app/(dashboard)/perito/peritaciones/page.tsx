...
export default function PeritoPeritacionesPage() {
  ...
  async function confirmarRecepcion(id: string) {
    ...
    setPeritaciones(prev => prev.map(p => p.id === id ? { ...p, estado: 'recibida', fecha_recepcion: new Date().toISOString() } : p))
    setConfirmando(null)
  }

  // Talleres únicos para filtro
      ...
      <div style={{ background: 'white', border: '1px solid #E2E6EC', borderRadius: 16, overflow: 'hidden' }}>
        {filtradas.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {filtradas.map(p => (
<tr key={p.id} onClick={() => router.push(`/perito/peritaciones/${p.id}`)} style={{ borderBottom: '1px solid #F7F8FA', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.background = '#F7F8FA')} onMouseLeave={e => (e.currentTarget.style.background = 'white')}>
                 <td style={{ padding: '14px 16px', fontFamily: 'DM Mono, monospace', fontWeight: 600, color: '#063940', fontSize: 12 }}>
                  ...
                  <td style={{ padding: '14px 16px', color: '#4A5568' }}>
                    {p.taller?.nombre_fantasia || <span style={{ color: '#C8D0DC' }}>—</span>}
                  </td>
                  <td style={{ padding: '14px 16px', fontFamily: 'DM Mono, monospace', fontSize: 12, color: '#4A5568' }}>
                    {p.nro_siniestro || <span style={{ color: '#C8D0DC' }}>—</span>}
                  </td>
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    {p.estado === 'enviada' ? (
                      <button
                        onClick={() => confirmarRecepcion(p.id)}
