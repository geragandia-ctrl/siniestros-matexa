...
export default function NuevaPeritacionPage() {
  ...
  function agregarFotos(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Reset input para poder agregar más
  ...
  async function guardar() {
    ...
    if (fotos.length > 0) {
      for (const foto of fotos) {
        ...
        if (errUpload) { console.error('Error subiendo foto:', errUpload); continue }
        ...
        <div style={{ background: 'white', border: '1px solid #E2E6EC', borderRadius: 18, padding: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <select
                value={companiaId} onChange={e => setCompaniaId(e.target.value)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                ...
                <input
                  type="text" value={vehiculo} onChange={e => setVehiculo(e.target.value)}
                  placeholder="Ej: VW Gol 2020"
              ...
              <div>
                ...
                <input
                  type="text" value={patente} onChange={e => setPatente(e.target.value.toUpperCase())}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                ...
                <input
                  type="text" value={nroSiniestro} onChange={e => setNroSiniestro(e.target.value)}
        ...
        <div style={{ background: 'white', border: '1px solid #E2E6EC', borderRadius: 18, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <button
              ...
              style={{ background: '#063940', color: 'white', border: 'none', borderRadius: 10, padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 6 }}
            >
              📷 Agregar fotos
            </button>
          </div>

          {/* Input oculto — en mobile abre cámara, en PC abre explorador */}
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
          ...
          {fotos.length === 0 ? (
            <div
              onClick={() => fileInputRef.current?.click()}
              style={{ border: '2px dashed #E2E6EC', borderRadius: 14, padding: '32px 20px', textAlign: 'center', cursor: 'pointer', transition: 'border-color .2s' }}
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#4A5568', marginBottom: 4 }}>
                Tocá para sacar fotos o subir desde la galería
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 10 }}>
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                style={{ border: '2px dashed #E2E6EC', borderRadius: 10, aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 24, color: '#C8D0DC', transition: 'all .2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = '#3e838c'; e.currentTarget.style.color = '#3e838c' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#E2E6EC'; e.currentTarget.style.color = '#C8D0DC' }}
              >
                +
              </div>
            </div>
